...
static map<string, string> g_WinPathsOfName;

static void printUsage(char *pExe)
{
	printf("\n");
	printf("Usage: %s [-id <uid> <gid>] [-log on | off] export_path>exportdir|export_path>exportdir\n", pExe);
	printf("For example:\n");
	printf("On Windows> %s d:\\one>/one|d:\\two>/two\n", pExe);
...
static void start(char *path)
{
	...
	//g_MountProg.Export(path, "a");  //export path for mount
	char winPath[MAXPATHLEN] = {0};
	char name[MAXPATHLEN] = {0};
	char *begin = path;
	while (*path != '\0' && (path - begin) < MAXPATHLEN )
	{
		int j, k;
		for (j = 0; *path != '\0' && j < MAXPATHLEN; path++, j++)
		{
			if (*path == '=')
			{
				path++;
		...
		winPath[j] = '\0';
		for (k = 0; *path != '\0' && k < MAXPATHLEN; path++, k++)
		{
			if (*path == ';')
			{
				name[k] = '\0';	
		...
		name[k] = '\0';
		if (name[0] && winPath[0])
			g_MountProg.Export(winPath, name); 
	}
